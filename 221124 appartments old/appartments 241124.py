...
def combined_task():
    ...
    scrape_result = scrape_text()
    urls_result = get_urls()
    combined_result = {
        "scrape_result": scrape_result,
        "url_result": urls_result,
    }
...
print(f"Scraped data at time: " + time.ctime() + "\n" + f"Initial Value = {initial_counts}")
print("Scheduler started. Polling every 5 minutes...")
schedule.every(5).minutes.do(combined_task)
# Keep the script running
while True:
    schedule.run_pending()
    if combined_result:
        print(time.ctime(), "No changes")
        difference = combined_result["url_result"].difference(initial_links)
        if int(combined_result["scrape_result"]) > initial_counts and difference is not None:
